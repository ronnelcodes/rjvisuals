...
export const handler = async event => {
  ...
  try{
    ...
    const signed=await Promise.all(photos.map(async p=>{const s=await supabase(`/storage/v1/object/sign/private-galleries/${p.storage_path}`,{method:'POST',body:JSON.stringify({expiresIn})});return{filename:p.filename,url:s.signedURL.startsWith('http')?s.signedURL:`${process.env.SUPABASE_URL}${s.signedURL}`};}));
