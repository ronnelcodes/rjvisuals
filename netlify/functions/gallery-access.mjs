import { json, supabase, verifyPassword, storageSignedUrl } from './_shared.mjs';
export const handler = async event => {
  ...
  try{
    ...
    const signed=await Promise.all(photos.map(async p=>{const s=await supabase(`/storage/v1/object/sign/private-galleries/${p.storage_path}`,{method:'POST',body:JSON.stringify({expiresIn})});return{filename:p.filename,url:storageSignedUrl(s.signedURL)};}));
    return json(200,{title:gallery.title,photos:signed,expiresIn,expiresAt:Date.now()+(expiresIn*1000)});
