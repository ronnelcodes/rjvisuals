...
document.querySelector('#upload-form').addEventListener('submit', async event => {
  ...
  try {
    ...
    for (const file of files) {
      status.textContent = `${optimize ? 'Optimizing and uploading' : 'Uploading'} ${++done} of ${files.length}…`;
      const prepared = optimize ? await optimizeGalleryPhoto(file) : { blob: file, filename: file.name, bytesSaved: 0 };
...
async function optimizeGalleryPhoto(file) {
  ...
  bitmap.close();
  const blob = await new Promise((resolve, reject) => canvas.toBlob(result => result ? resolve(result) : reject(new Error(`Could not optimize ${file.name}`)), 'image/webp', 0.88));
  if (blob.size >= file.size) return { blob: file, filename: file.name, bytesSaved: 0 };
